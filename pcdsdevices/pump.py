...
class GammaController(BaseInterface, Device):
    ...
    power_autosave = Cpt(EpicsSignal, ":ASPOWERE", write_pv=":ASPOWEREDES", kind="config")
    high_voltage_enable_autosave = Cpt(EpicsSignal, ":ASHVE", write_pv=":ASHVEDES", kind="normal")
...
class IonPumpBase(BaseInterface, Device):
    ...
    __doc__ = (__doc__ % IonPump_base).replace("Ion Pump", "Ion Pump Base Class")
    ...
    state = Cpt(EpicsSignal, ":STATEMON", write_pv=":STATEDES", kind="normal", string=True)
    ...
    calfactor = Cpt(EpicsSignal, ":CALFACTORDES", write_pv=":CALFACTOR", kind="config")
...
class PIPPLC(Device):
    ...
    high_voltage_in = Cpt(EpicsSignalRO, ":HV_DI_RBV", kind="normal", doc="high voltage digital input")
    interlock_ok = Cpt(EpicsSignalRO, ":ILK_OK_RBV", kind="normal", doc="interlock  is ok when true")
    protection_setpoint = Cpt(EpicsSignalWithRBV, ":AT_VAC_SP", kind="config", doc="Protection/At Vacuum Setpoint")
    setpoint_hysteresis = Cpt(EpicsSignalWithRBV, ":SP_HYS", kind="config", doc="Protection Setpoint Hysteresis")
    pump_on_status = Cpt(EpicsSignalRO, ":HV_DI_RBV", kind="normal", doc="ion pump output state")
    pump_state = Cpt(EpicsSignalRO, ":STATE_RBV", kind="hinted")
    at_vac_setpoint = Cpt(EpicsSignalWithRBV, ":AT_VAC_SP", kind="omitted", doc="at vacuum set point")
    high_voltage_switch = Cpt(
        EpicsSignalWithRBV, ":HV_SW", kind="config", doc="epics command to switch on the high voltage"
    )
    plc_ai_offset = Cpt(
        EpicsSignalRO,
        ":AI_Offset_RBV",
        kind="config",
        doc=("Analog input offset must match ion pump analog ouput offset. Default: 13"),
    )
    auto_on = Cpt(
        EpicsSignalWithRBV,
        ":Auto_On",
        kind="config",
        doc=(
            "Setting to automatically turn on the ion pump when the"
            "reference gauge pressure is below protection "
            "setpoint"
        ),
    )
    override_status = Cpt(EpicsSignalRO, ":OVRD_ON", kind="omitted", doc="Regional Override Status")
    override_force_on = Cpt(EpicsSignalWithRBV, ":FORCE_START", kind="omitted", doc="Force Ion Pump to turn on")
    qpc_name = Cpt(EpicsSignalRO, ":VPCNAME", kind="config", doc="Quad Ion Pump Controller Name")
    qpc_pumpsize = Cpt(EpicsSignalRO, ":PUMPSIZE", kind="config", doc="Ion Pump Size (L/s)")
    interlock_device = Cpt(
        EpicsSignalRO,
        ":ILK_DEVICE_RBV",
        kind="config",
        string=True,
    ...
    autoOn_countdown = Cpt(
        EpicsSignalRO, ":AutoOn_timer_RBV", kind="normal", doc="Timer count down to turn on the ion pump "
    )
...
class PTMPLC(Device):
    ...
    interlock_status = Cpt(EpicsSignalRO, ":ILK_OK_RBV", kind="normal", doc="interlock  is ok when true")
...
class AgilentSerial(Device):
    ...
    vent_valve_operation = Cpt(EpicsSignal, ":VENT_VALVE_OPERATION", kind="omitted")
...
class Navigator(AgilentSerial):
    ...
    vent_open_time_raw = Cpt(EpicsSignalRO, ":VENT_OPEN_TIME_RAW_RBV", kind="omitted")
    ...
    sp_write_press_unit = Cpt(EpicsSignalRO, ":SP_WRITE_PRESS_UNIT_RBV", kind="omitted")
    stop_speed_reading = Cpt(EpicsSignalRO, ":STOP_SPEED_READING_RBV", kind="omitted")
    ctrl_heatsink_temp = Cpt(EpicsSignalRO, ":CTRL_HEATSINK_TEMP_RBV", kind="omitted")
