...
class K6514(BaseInterface, Device):
    ...
    avg_mode = Cpt(
        EpicsSignal, ":PutAvgType", string=True, kind="normal", doc="Select filter control: Moving or Repeated"
    )
    ...
    integration_time = Cpt(
        EpicsSignal, ":GetIntCycles", write_pv=":PutIntCycles", kind="hinted", doc="Set integration time"
    )
    measurement_function = Cpt(
        EpicsSignal,
        ":PutFunction",
        string=True,
        kind="hinted",
        doc="Select function - Voltage, Current, Resistance, or Charge",
    )
...
class K2700(BaseInterface, Device):
    ...
    idn = Cpt(EpicsSignalRO, ":Identity", kind="normal", doc="Identity (name) of this device")
    reading = Cpt(EpicsSignalRO, ":Reading", kind="normal", doc="Trigger and return a new measurement")
    dcv_range = Cpt(EpicsSignalRO, ":GetDCV", kind="normal", doc="DC voltage range")
    acv_range = Cpt(EpicsSignalRO, ":GetACV", kind="normal", doc="AC voltage range")
    dci_range = Cpt(EpicsSignalRO, ":GetDCI", kind="normal", doc="DC current range")
    aci_range = Cpt(EpicsSignalRO, ":GetACI", kind="normal", doc="AC current range")
