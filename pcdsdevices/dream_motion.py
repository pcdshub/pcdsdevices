...
class DREAM_SL3K4(BaseInterface, GroupDevice):
    ...
    blades_drift = Cpt(
        PytmcSignal, ":aBladesfDrift", io="i", kind="hinted", doc="Array of blade drifts in  order [X1, X2, Y1, Y2]"
    )
    blades_drifted = Cpt(
        PytmcSignal, ":aBladesfDrifted", io="i", kind="hinted", doc="Blades drift status in  order [X1, X2, Y1, Y2]"
    )
    ...
    ref_refresh = Cpt(
        PytmcSignal, ":REFRENCING:REFRESH", io="io", kind="hinted", doc="Must-reference re-home interval in seconds"
    )
    must_home = Cpt(PytmcSignal, ":MUSTHOME:ACTIVE", io="i", kind="hinted", doc="Re-home mandatory, move blocked")
    all_homed = Cpt(PytmcSignal, ":ALLHOMED", io="i", kind="hinted", doc="TRUE when all blades report homed")
