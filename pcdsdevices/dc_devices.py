...
class ICTChannel(Device):
    ...
    ch_current = FCpt(EpicsSignalRO, "{self.prefix}:Output{self._ch}:Current", kind="hinted")
    ch_status = FCpt(
        EpicsSignal,
    ...
    ch_name = FCpt(EpicsSignal, "{self.prefix}:Output{self._ch}.DESC", kind="hinted")
    breaker_status = FCpt(EpicsSignalRO, "{self.prefix}:Output{self._ch}:BreakerStatus", kind="hinted", string=True)
...
class ICTBus(Device):
    ...
    bus_current = FCpt(EpicsSignalRO, "{self.prefix}:Bus{self._bus}:Current", kind="hinted")
    bus_voltage = FCpt(EpicsSignalRO, "{self.prefix}:Bus{self._bus}:Voltage", kind="hinted")
    bus_name = FCpt(EpicsSignal, "{self.prefix}:Bus{self._bus}.DESC", kind="hinted")
