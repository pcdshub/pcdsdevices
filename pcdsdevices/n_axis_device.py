...
from .epics_motor import BeckhoffAxis, IMS, Newport, PMC100
from .epics_motor import PCDSMotorBase, SmarAct
    ...
    for cpt, cpt_pv, _type in motor_types:
        ...
        for i in range(1, max_axis+1):
            ...
            for n in range(1, i+1):
                comp = Cpt(_type, ':{}:{:02d}'.format(cpt_pv, n),
                           kind='normal')
                axs['m{}'.format(n)] = comp
...
def get_n_axis_device(prefix, n_axis, *args, name, motor_type='Beckhoff',
                      **kwargs):
