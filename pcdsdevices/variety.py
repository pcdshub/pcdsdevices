...
def _length_validate(min_count, max_count, type_):
    ...
    def validate(value):
        ...
        if min_count == max_count:
            raise ValueError(f"Expecting a sequence of {min_count} values ({type_})")

        raise ValueError(f"Expecting a sequence betweeen {min_count} and {max_count} values ({type_})")
...
_default_bitmask_style = dict(shape="rectangle", on_color="green", off_color="gray")
    ...
    "bitmask": schema.Schema(
        {
            ...
            Optional("orientation", default="horizontal"): schema.Or("horizontal", "vertical"),
            Optional("bits", default=8): int,
            Optional("first_bit", default="most-significant"): schema.Or("most-significant", "least-significant"),
            Optional("meaning", default=None): [str],
            Optional("style", default=_default_bitmask_style): schema.Schema(
                {
                    Optional("shape", default="rectangle"): schema.Or("circle", "rectangle"),
            ...
            Optional("range"): schema.Schema(
                {
                    ...
                    Optional("source", default="use_limits"): schema.Or("use_limits", "value"),
                }
            ),
            Optional("delta"): schema.Schema(
                {
                    ...
                    Optional("source", default="value"): schema.Or("signal", "value"),
                    Optional("signal"): schema.Or(str, ophyd.Component),
                    Optional("add_signal"): str,
                    Optional("adds_to", default="setpoint"): schema.Or("setpoint", "readback", "custom-signal"),
                }
            ),
            **common_schema,
        }
    ...
    "text": schema.Schema(
        {
            ...
            Optional("encoding", default="utf-8"): schema.Or("utf-8", "latin-1", "ascii"),
            Optional("format", default="plain"): schema.Or("plain", "markdown", "html"),
            **common_schema,
        }
    ),
    "enum": schema.Schema(
        {"variety": schema.Or(*varieties_by_category["enum"]), Optional("enum_strings"): [str], **common_schema}
    ),
}
...
def expand_dotted_dict(root):
    ...
    def expand_key(dct, key, value):
        ...
        for part in parts:
            if not part:
                raise ValueError(f"Dotted key cannot contain empty part ({key})")

            if part not in dct:
                dct[part] = {}
            elif not isinstance(dct[part], dict):
                raise ValueError(f"Dotted key does not refer to a dictionary ({part} of {key})")
...
def validate_metadata(md):
    ...
    try:
        ...
        raise ValueError("Unexpected metadata keys without variety specified: " + ", ".join(md)) from None
    ...
    try:
        ...
        raise ValueError(f"Unexpected variety: {variety!r}.  Valid options: " + ", ".join(_schema_registry)) from None
