...
class CalibrationAxe(TwinCATStatePMPS):
    """
...
class SCaliFoil(BaseInterface, GroupDevice, LightpathMixin):
    ...
    at2k4 = Cpt(CalibrationAxe, 'AT2K4:IP1:STATE', add_prefix=(), kind='normal')
    at2k4_y = Cpt(BeckhoffAxis, ':MMS', doc="Y-axis of sample caliration at2k4", kind='normal')
    removed = False
    transmission = 1
    SUB_STATE = 'state'

    # dummy signal, state is always the same
    lightpath_cpts = ['at2k4.user_readback']
