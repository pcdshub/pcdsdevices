...
class PneumaticActuator(InOutPositioner):
    ...
    def __init__(self, prefix, *, name, transmission=1, filter_type="Unknown filter", **kwargs):
...
class RTDSX0ThreeStage(BaseInterface, LightpathMixin):
    ...
    open_limit = Cpt(EpicsSignalRO, ":MMS:Y.HLS", kind="normal", doc="Reads if the y-stage is at its outer limit.")
    lightpath_cpts = ["open_limit"]

    def calc_lightpath_state(self, open_limit=None):
        # Logic, calculations using open_limit for y-stage
        status = LightpathState(inserted=not open_limit, removed=open_limit, output={"K0": 0.0})
        return status
