...
class HE_LODCMEnergy(FltMvInterface, PseudoPositioner):
    energy = Cpt(
        ...
        limits=(4, 25),
        verbose_name="",
        doc=("PseudoSingle that moves the calculated LODCM selected energy in keV."),
    )
    ...
    def get_crystal(self) -> Crystal:
        ...
        for crystal in (SI111, SI220):
            if (
                crystal.t2ty_range[0] <= rbv_t2ty1 <= crystal.t2ty_range[1]
    ...
    def _energy_from_theta(self, theta_deg: float, crystal: Crystal) -> float:
        ...
        if grating_period is not None:
            grating_correction = np.sqrt(
                1
                + (2 * crystal.d_spacing / grating_period) ** 2
    ...
    def _pos_from_energy(self, energy: float) -> tuple[float, float]:
        ...
        gap_t2tz = GAP * (
            np.cos(2 * bragg_rad - grating_angle_rad)
            * np.sin(bragg_rad)
            / np.sin(2 * bragg_rad)
            / np.sin(bragg_rad - grating_angle_rad)
        )

        grating_t2tz = 1 - np.sin(bragg_rad) * np.sin(2 * bragg_rad - grating_angle_rad) / np.sin(
            2 * bragg_rad
        ) / np.sin(bragg_rad - grating_angle_rad)
    ...
    def _get_real_positioners(cls) -> list:
        ...
        return [(attr, cpt) for attr, cpt in super()._get_real_positioners() if attr not in exclude]
    ...
    def forward(self, pseudo_pos: Any) -> Any:
        ...
        return self.RealPosition(t1ry=th, t2ry=th, t2tz=t2tz)
...
class HE_LODCM(BaseInterface, GroupDevice):
    ...
    def format_status_info(self, status_info):
        ...
        def motor_status(attr, status_info):
            units = get_status_value(status_info, attr, "user_setpoint", "units")
            user = get_status_float(status_info, attr, "position")
            dial = get_status_float(status_info, attr, "dial_position", "value")
        ...
        def lakeshore_status(attr, channel, status_info):
            units = get_status_value(status_info, attr, f"temp_{channel}", "units", "value")
            temp = get_status_float(status_info, attr, f"temp_{channel}", "temp", "value")
            return units, temp
        ...
        {form(f"tn [{t2tn_units}]", "", f"{t2tn_user} ({t2tn_dial})")}
