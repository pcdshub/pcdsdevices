...
def fake_ipm():
    ...
    ipm.diode.state.state.sim_set_enum_strs(["Unknown"] + InOutRecordPositioner.states_list)
...
def fake_ipm_with_box(request):
    ...
    if request.param is IPM_IPIMB:
        ipm = FakeIPM("Test:My:IPM", name="test_ipm", prefix_ipimb="test_ipimb")
    else:
        ipm = FakeIPM("Test:My:IPM", name="test_ipm", prefix_wave8="test_wave8")
    ipm.diode.state.state.sim_put(0)
    ipm.diode.state.state.sim_set_enum_strs(["Unknown"] + InOutRecordPositioner.states_list)
    ipm.target.state.sim_put(0)
    ipm.target.state.sim_set_enum_strs(["Unknown"] + IPMTarget.states_list)
...
def test_ipm_factory():
    ...
    assert isinstance(IPM("IPM2", name="ipm2", prefix_ipimb="ipimb", prefix_ioc="ioc"), IPM_IPIMB)
    assert isinstance(IPM("IPM3", name="ipm3", prefix_wave8="wave8"), IPM_Wave8)
