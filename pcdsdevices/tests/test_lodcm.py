...
def make_fake_offset_ims(prefix, motor_pos=0, user_offset=0):
    ...
    fake_ims = make_fake_device(MyOffsetIMS)("PREFIX", motor_prefix="MOTOR:PREFIX", name="fake_ims")
...
def fake_tower1():
    ...
    tower1.h1n_state.state.sim_set_enum_strs(["Unknown"] + H1N.states_list)
    # set y1_state to 'C'
    tower1.y1_state.state.sim_put(1)
    tower1.y1_state.state.sim_set_enum_strs(["Unknown"] + Y1.states_list)
    # set chi1_state to 'C'
    tower1.chi1_state.state.sim_put(1)
    tower1.chi1_state.state.sim_set_enum_strs(["Unknown"] + CHI1.states_list)
    return tower1
...
def fake_tower2():
    ...
    tower2.y2_state.state.sim_set_enum_strs(["Unknown"] + Y2.states_list)
    # set chi2_state to 'C'
    tower2.chi2_state.state.sim_put(1)
    tower2.chi2_state.state.sim_set_enum_strs(["Unknown"] + CHI2.states_list)
    # set h2n_state to 'C'
    tower2.h2n_state.state.sim_put(1)
    tower2.h2n_state.state.sim_set_enum_strs(["Unknown"] + H2N.states_list)
...
def test_move_energy(fake_lodcm):
    ...
    with patch("pcdsdevices.lodcm.LODCM.get_material", return_value="Si"):
...
def test_get_reflection_tower1(fake_tower1):
    ...
    res = tower1.get_reflection()
    assert res == (2, 2, 2)
    with patch("pcdsdevices.lodcm.CrystalTower1.is_diamond", return_value=None):
        with patch("pcdsdevices.lodcm.CrystalTower1.is_silicon", return_value=None):
            with pytest.raises(ValueError):
...
def test_get_reflection_tower2(fake_tower2):
    ...
    res = tower2.get_reflection()
    assert res == (2, 0, 0)
    with patch("pcdsdevices.lodcm.CrystalTower2.is_diamond", return_value=None):
        with patch("pcdsdevices.lodcm.CrystalTower2.is_silicon", return_value=None):
            with pytest.raises(ValueError):
...
def test_calc_geometry_c(fake_energy_c):
    ...
    with patch("pcdsdevices.lodcm.LODCMEnergyC.get_reflection", return_value=(1, 1, 1)):
    ...
    with patch("pcdsdevices.lodcm.LODCMEnergyC.get_reflection", return_value=(2, 2, 0)):
        th, z = energy.calc_geometry(energy=10)
...
def test_calc_geometry_si(fake_energy_si):
    ...
    with patch("pcdsdevices.lodcm.LODCMEnergySi.get_reflection", return_value=(1, 1, 1)):
        th, z = energy.calc_geometry(energy=10)
    ...
    with patch("pcdsdevices.lodcm.LODCMEnergySi.get_reflection", return_value=(2, 2, 0)):
        th, z = energy.calc_geometry(energy=10)
