...
warnings.filterwarnings("ignore", message="Signal.put no longer takes keyword arguments")
...
def check_value(self, value):
    ...
    if not (low_limit <= value <= high_limit):
        raise LimitError("Value {} outside of range: [{}, {}]".format(value, low_limit, high_limit))
...
def find_pcdsdevices_submodules() -> dict[str, ModuleType]:
    ...
    for item in pkgutil.walk_packages(path=[package_root], prefix="pcdsdevices."):
        try:
            modules[item.name] = sys.modules[item.name]
        except KeyError:
            # Submodules may not yet be imported; do that here.
            try:
                modules[item.name] = importlib.import_module(item.name, package="pcdsdevices")
            except Exception:
...
def find_all_classes(classes, skip: Optional[list[str]] = None) -> list[Any]:
    ...
    return sorted(set(devices), key=sort_key)


def find_all_device_classes(skip: Optional[list[str]] = None) -> list[ophyd.Device]:
    """
    Find all device classes in pcdsdevices and return them as a list.
...
def find_all_callables() -> list[Callable]:
    ...
    def should_include(obj):
        ...
        return (
            ...
            and module.startswith("pcdsdevices")
            and not module.startswith("pcdsdevices._version")
            and not module.startswith("pcdsdevices.tests")
...
def best_effort_instantiation(device_cls, *, skip_on_failure=True):
    ...
    for param in sig.parameters.values():
        if param.default is inspect.Signature.empty:
            if param.kind not in {param.VAR_KEYWORD, param.VAR_POSITIONAL}:
                ...
                kwargs.setdefault(param.name, "test:abcd" if "prefix" in param.name else "test")
    ...
    try:
        ...
        if skip_on_failure:
            pytest.skip(f"Unable to instantiate {device_cls}: {ex} (kwargs={kwargs})")
        raise
