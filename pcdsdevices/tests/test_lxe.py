...
from ..lxe import LaserEnergyPlotContext, LaserEnergyPositioner, LaserTiming, LaserTimingCompensation, Lcls2LaserTiming
...
def test_laser_energy_positioner(monkeypatch, lxe_calibration_file):
    ...
    lxe = MyLaserEnergyPositioner("", calibration_file=lxe_calibration_file, name="lxe")
    lxe.move(10)
...
def test_laser_timing_motion(lxt):
    ...
    for pos in range(1, 3):
        ...
        np.testing.assert_allclose(-lxt._fs_tgt_time.get(), convert_unit(pos, "s", "ns"))
    ...
    for pos, offset in [(1, 1), (3, 2), (2, -1)]:
        ...
        np.testing.assert_allclose(lxt.setpoint.forward(pos), convert_unit(-(pos - offset), "s", "ns"))
        ...
        lxt.move(pos).wait(1)
        np.testing.assert_allclose(lxt.position, pos)
        np.testing.assert_allclose(-lxt._fs_tgt_time.get(), convert_unit(pos - offset, "s", "ns"))
...
def test_laser_timing_offset(lxt):
    ...
    for pos in [1.0e-6, 2.0e-6, 8.0e-4]:
        ...
        np.testing.assert_allclose(lxt.limits, (pos + initial_limits[0], pos + initial_limits[1]))
...
def lxt_ttc(monkeypatch):
    ...
    lxt_ttc = make_fake_device(LaserTimingCompensation)(
        "", delay_prefix="DELAY:", laser_prefix="LASER:", name="lxt_ttc"
    )
...
def test_laser_timing_compensation(lxt_ttc):
    ...
    np.testing.assert_allclose(
        [lxt_ttc.delay.forward(i * 1e-6).motor for i in range(len(milliseconds_to_mm_values))],
        milliseconds_to_mm_values,
    )

    np.testing.assert_allclose(
        [lxt_ttc.delay.inverse(i * 1e5).delay for i in range(len(mm_to_seconds_values))],
        mm_to_seconds_values,
    )
...
def test_lxe_disconnected(lxe_calibration_file):
    ...
    LaserEnergyPositioner("TST2", name="tst2", calibration_file=lxe_calibration_file)
    LaserTimingCompensation("TST3", delay_prefix="TST3:DELAY", laser_prefix="TST3:LASER", name="tst3")
