...
class LimCls(PVStatePositioner):
    ...
    _state_logic = {"lowlim": {0: "in", 1: "defer"}, "highlim": {0: "out", 1: "defer"}}
...
def test_twincat_state_config_dynamic():
    ...
    def check_class(cls, state_count, motor_count=1):
        assert cls.config.kwargs["state_count"] == state_count, (
            f"Found the wrong state count for {cls}, must be some error related to UpdateComponent."
        )
        assert cls.config.kwargs["motor_count"] == motor_count, (
            f"Found the wrong motor count for {cls}, must be some error related to UpdateComponent."
        )
        assert len(cls.state.kwargs["enum_attrs"]) == state_count + 1, (
            f"Found the wrong number of enum_attrs for {cls}, something must have gone wrong in __init_subclass__."
        )
