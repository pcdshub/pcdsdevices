...
def fake_mpod_hv_channel():
    ...
    mpod_hv_channel = FakeMPODChannel("TEST:MPOD:CHANNEL:HV", "TEST:MPOD:CARD", name="test")
    # switch Off
...
def test_mpod_channel_factory(fake_mpod_channel_v30, fake_mpod_channel_v500):
    with patch("pcdsdevices.mpod.MPODChannel", return_value=fake_mpod_channel_v30):
        hv = MPOD("TST:MY:MMS:CH:100", name="test_hv_mpod")
        assert isinstance(hv, MPODChannelLV)
    with patch("pcdsdevices.mpod.MPODChannel", return_value=fake_mpod_channel_v500):
        hv = MPOD("TST:MY:MMS:CH:100", name="test_hv_mpod")
