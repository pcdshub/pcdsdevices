...
class LCLSItem(OphydItem):
    name = EntryInfo(
        ("Shorthand Python-valid name for the Python instance. Must be between 3 and 80 characters."),
        optional=False,
        enforce=re.compile(r"[a-z][a-z\_0-9]{2,78}$"),
    )
    beamline = EntryInfo("Section of beamline the device belongs", optional=False, enforce=str)
    location_group = EntryInfo("LUCID grouping parameter for location", optional=False, enforce=str)
    functional_group = EntryInfo("LUCID grouping parameter for function", optional=False, enforce=str)
    ...
    lightpath = EntryInfo("If the device should be included in the LCLS Lightpath", enforce=bool, default=False)
    input_branches = EntryInfo(("List of branches the device can receive beam from."), optional=True, enforce=list)
    output_branches = EntryInfo(("List of branches the device can deliver beam to."), optional=True, enforce=list)
    ioc_engineer = EntryInfo(("Engineer for the IOC. Used to build IOC configs."), optional=True, enforce=str)
    ioc_location = EntryInfo("Location of the IOC. Used to build IOC configs.", optional=True, enforce=str)
    ioc_hutch = EntryInfo(("Hutch the IOC will be used in. Used to build IOC configs."), optional=True, enforce=str)
    ioc_release = EntryInfo(
        ("Full path to IOC release directory. Used to build IOC configs."), optional=True, enforce=str
    )
    ioc_arch = EntryInfo(("The IOC host architecture. Used to build IOC configs."), optional=True, enforce=str)
    ioc_name = EntryInfo("The name of the device IOC. Used to build the IOC.", optional=True, enforce=str)
    ioc_type = EntryInfo(
        (
...
class LegacyItem(HappiItem):
    ...
    prefix = EntryInfo("A base PV for all related records", optional=False, enforce=str)
    beamline = EntryInfo("Section of beamline the device belongs", optional=False, enforce=str)
    location_group = EntryInfo("Grouping parameter for device location", optional=False, enforce=str)
    functional_group = EntryInfo("Grouping parameter for device function", optional=False, enforce=str)
    z = EntryInfo("Beamline position of the device", enforce=float, default=-1.0)
    stand = EntryInfo(
        "Acronym for stand, must be three alphanumeric "
        "characters like an LCLSI stand (e.g. DG3) or follow "
    ...
    detailed_screen = EntryInfo("The absolute path to the main control screen", enforce=str)
    embedded_screen = EntryInfo("The absolute path to the embedded control screen", enforce=str)
    engineering_screen = EntryInfo("The absolute path to the engineering control screen", enforce=str)
    system = EntryInfo("The system the device is involved with, i.e Vacuum, Timing e.t.c", enforce=str)
    macros = EntryInfo(
        "The EDM macro string asscociated with the "
        "with the device. By using a jinja2 template, "
        "this can reference other EntryInfo keywords.",
        enforce=str,
    )
    lightpath = EntryInfo("If the device should be included in the LCLS Lightpath", enforce=bool, default=False)
    documentation = EntryInfo("Relevant documentation for the Device", enforce=str)
    parent = EntryInfo("If the device is a component of another, enter the name", enforce=str)
    ...
    def screen(self):
        warnings.warn(
            "The 'screen' keyword is no longer used in Happi as it "
...
class Attenuator(BeamControl):
    ...
    n_filters = EntryInfo("Number of filters on the Attenuator", enforce=int, optional=False)
...
class OffsetMirror(BeamControl):
    ...
    pitch_ranges = EntryInfo(
        "valid pitch ranges for each destination", optional=True, enforce=list, include_default_as_kwarg=False
    )
    x_ranges = EntryInfo(
        "valid x positions, determining insertion", optional=True, enforce=list, include_default_as_kwarg=False
    )
    y_ranges = EntryInfo(
        "valid y positions, determining coating", optional=True, enforce=list, include_default_as_kwarg=False
    )
...
class LODCM(BeamControl):
    ...
    mono_line = EntryInfo("Name of the MONO beamline", enforce=str, optional=False)
    kwargs = deepcopy(BeamControl.kwargs)
    kwargs.default.update({"mono_line": "{{mono_line}}", "main_line": "{{beamline}}"})
...
class Elliptec(LCLSItem):
    ...
    ioc_serial = EntryInfo(
        ("Serial number of the stage controller. Used to build IOC configs."), optional=True, enforce=str
    )
    ioc_model = EntryInfo(("Model number of the stage. Used to build IOC configs."), optional=True, enforce=str)
    ioc_channel = EntryInfo(
        ("Channel number of the stage. Used to build IOC configs, and passed to python object."),
        optional=False,
        enforce=str,
    )
    ioc_base = EntryInfo(
        (
            "Elliptec controller PV base. This is not "
    ...
    ioc_alias = EntryInfo("Optional alias to give the axis.", optional=True, enforce=str, default=None)
...
class Qmini(LCLSItem):
    ...
    ioc_serial = EntryInfo(
        ("Serial number of the spectrometer. Used to build IOC configs."), optional=True, enforce=str
    )
    ioc_use_evr = EntryInfo(
        ('Option to use an EVR or not. Options are "yes" and "no". Default is no.'),
        optional=True,
        default="no",
        enforce=str,
    )
    ioc_evr_channel = EntryInfo("The EVR channel the IOC will use, if any.", optional=True, default="1", enforce=str)
...
class SmarActMotor(LCLSItem):
    ...
    ioc_ip = EntryInfo(
        ("Netconfig entry name for motor controller. Used to build IOC configs."), optional=True, enforce=str
    )
    ioc_base = EntryInfo(("Base PV of the SmarAct controller. Used to build IOC configs."), optional=True, enforce=str)
    ioc_channel = EntryInfo(
        ("Controller channel that the axis is on. Ranges from 1 to 18."), optional=True, enforce=str
    )
    ioc_alias = EntryInfo("Optional PV alias to give this axis.", optional=True, enforce=str)
...
class SmarActTipTiltMotor(LCLSItem):
    ...
    ioc_tip_channel = EntryInfo(
        ("Controller channel that the tip axis is on. Ranges from 1 to 18."), optional=True, enforce=str
    )
    ioc_tilt_channel = EntryInfo(
        ("Controller channel that the tilt axis is on. Ranges from 1 to 18."), optional=True, enforce=str
    )
    ioc_base = EntryInfo(("Base PV of the SmarAct controller. Used to build IOC configs."), optional=True, enforce=str)
    ioc_alias = EntryInfo(("Base alias PV of the two stages. Used to build IOC configs."), optional=True, enforce=str)
    ioc_tip_suffix = EntryInfo("PV suffix to give the tip axis alias.", default="_TIP1", optional=True, enforce=str)
    ioc_tilt_suffix = EntryInfo("PV suffix to give the tilt axis alias.", default="_TILT1", optional=True, enforce=str)
...
class BaslerCamera(LCLSItem):
    ...
    ioc_ip = EntryInfo(
        ("IP address (not netconfig name) of the camera. Used to build IOC configs."), optional=True, enforce=str
    )
    ioc_cam_model = EntryInfo(
        ('Model number of the camera, e.g. "Basler_acA2500-20gm". Used to build IOC config files.'),
        optional=True,
        enforce=str,
    )
    ioc_use_evr = EntryInfo(
    ...
    ioc_evr_channel = EntryInfo("The EVR channel the IOC will use, if any.", optional=True, default="1", enforce=str)
    ioc_net_if = EntryInfo(
        ("Network interface name. Typically ETH0, ETH1, ENO1 or ENO2."), optional=True, default="ETH0", enforce=str
    )
    ioc_net_if_num = EntryInfo("Network interface number. Typically 1.", optional=True, default="1", enforce=str)
    ioc_http_port = EntryInfo("Unique HTTP port. Needed for MJPG plugin.", optional=True, default="7801", enforce=str)
...
class ThorlabsWfs(LCLSItem):
    ...
    ioc_model = EntryInfo("Thorlabs camera model number.", optional=True, default="WFS-40", enforce=str)
    ioc_id_num = EntryInfo("Unique ID number of the camera", optional=True, enforce=str)
    ioc_lenslet_pitch = EntryInfo("Pitch of WFS lenslets.", optional=True, default="150", enforce=str)
    ioc_use_evr = EntryInfo(
        ('Option to use an EVR or not. Options are "yes" and "no". Default is no.'),
        optional=True,
        default="no",
...
class ThorlabsPM101PowerMeter(LCLSItem):
    ...
    ioc_ip = EntryInfo(
        ("Netconfig name of the EK9000 the power meter is connected to. Used to build IOC configs."),
        optional=True,
        enforce=str,
    )
    ioc_card_num = EntryInfo(("Card number of the EL3174 the power meter is connected to."), optional=True, enforce=str)
    ioc_chan_num = EntryInfo(
        ("Channel number of the EL3174 the power meter is connected to."), optional=True, enforce=str
    )
    ioc_base = EntryInfo("Base PV of the EK9000 IOC", optional=True, enforce=str)
    ioc_alias = EntryInfo("Optional PV alias to give this power meter.", optional=True, enforce=str)
...
class EnvironmentalMonitor(LCLSItem):
    ...
    ioc_type.default = "EK9000"
    ioc_ip = EntryInfo(
        ("Netconfig name of the EK9000 the sensors are connected to. Used to build IOC configs."),
        optional=True,
        enforce=str,
    )
    ioc_base = EntryInfo("Base PV of the EK9000 IOC", optional=True, enforce=str)
    ioc_chan_num = EntryInfo("Channel number for the environmental monitor.", optional=True, enforce=str)
    ioc_card_num = EntryInfo("Card number for the environmental monitor.", optional=True, enforce=str)
...
class SimpleShutter(LCLSItem):
    ...
    ioc_base = EntryInfo("Base PV of the EK9000 IOC", optional=True, enforce=str)
    ioc_alias = EntryInfo("Optional PV alias for this shutter.", optional=True, enforce=str)
    ioc_chan_num = EntryInfo("Channel number for the shutter on the relay.", optional=True, enforce=str)
    ioc_card_num = EntryInfo("Card number for the DC relay.", optional=True, enforce=str)


class Leviton(LCLSItem):
    kwargs = deepcopy(LCLSItem.kwargs)
    kwargs.default["elevations"] = "{{elevations}}"
    elevations = EntryInfo(doc="List of elevation numbers for rack", optional=False, enforce=list)
