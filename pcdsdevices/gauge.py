...
class GaugeColdCathode(BaseGauge):
    ...
    relay_setpoint = Cpt(EpicsSignal, ":PSTATSPRBCK", write_pv=":PSTATSPDES", kind="normal")
    relay_enable = Cpt(EpicsSignal, ":PSTATENRBCK", write_pv=":PSTATEN", kind="normal")
    control_setpoint = Cpt(EpicsSignal, ":PCTRLSPRBCK", write_pv=":PCTRLSPDES", kind="normal")
    control_enable = Cpt(EpicsSignal, ":PCTRLENRBCK", write_pv=":PCTRLEN", kind="normal")
    protection_setpoint = Cpt(EpicsSignal, ":PPROTSPRBCK", write_pv=":PPROTSPDES", kind="normal")
    protection_enable = Cpt(EpicsSignal, ":PPROTENRBCK", write_pv=":PPROTEN", kind="normal")
...
class GaugeSetMks(GaugeSetBase):
    ...
    __doc__ = (__doc__ % GaugeSet_base).replace("Set", "Set w/o Pirani, but with controller")
...
class GaugeSetPiraniMks(GaugeSetPirani):
    ...
    __doc__ = (__doc__ % GaugeSet_base).replace("Set", "Set including the controller")
...
class GaugePLC(Device):
    ...
    state = Cpt(EpicsSignalRO, ":STATE_RBV", kind="hinted", doc="state of the gauge")


class GCCPLC(GaugePLC):
    """Class for a Cold Cathode Gauge controlled by PLC."""

    high_voltage_on = Cpt(EpicsSignalWithRBV, ":HV_SW", kind="normal", doc="command to switch the high voltage on")
    high_voltage_disable = Cpt(
        EpicsSignalRO, ":HV_DIS_DO_RBV", kind="normal", doc=("enables the high voltage on the cold cathode gauge")
    )
    protection_setpoint = Cpt(
        EpicsSignalWithRBV,
        ":PRO_SP",
        kind="normal",
    ...
    autoOn_countdown = Cpt(
        EpicsSignalRO, ":AutoOn_timer_RBV", kind="normal", doc="timer count down to turn on the gauge "
    )
...
class GFSPLC(GCCPLC):
    ...
    fault_setpoint_FS = Cpt(
        EpicsSignalRO, ":PSTATSPRBCK_FS", kind="config", doc="Vacuum fault setpoint for Fast shutter valve"
    )
...
class GHCPLC(GaugePLC):
    ...
    high_voltage_on = Cpt(EpicsSignalWithRBV, ":HV_SW", kind="normal", doc="command to switch the high voltage on")
    high_voltage_disable = Cpt(
        EpicsSignalRO, ":HV_DIS_DO_RBV", kind="normal", doc=("enables the high voltage on the hot cathode gauge")
    )
    protection_setpoint = Cpt(
        EpicsSignalWithRBV,
        ":PRO_SP",
...
def GaugeSet(prefix, *, name, index, **kwargs):
    ...
    if onlyGCC:
        if "prefix_controller" in kwargs:
            return GaugeSetMks(
                prefix, name=name, index=index, prefix_controller=kwargs.pop("prefix_controller"), **kwargs
            )
        else:
