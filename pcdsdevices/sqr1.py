"""
Ophyd class for square-one tri-sphere motion system (https://www.sqr-1.com/tsp.html) # noqa: E501

This module defines Ophyd (python based hardware abstraction and interfacing library) classes
for controlling a square-one tri-sphere motion system. The motion system consists of multiple
axes, each of which can be controlled individually. The axes include X, Y, Z, rX, rY, and rZ.
...
class SQR1Axis(PVPositionerIsClose):
    """
    This class represents and controls a single axis of the square-one tri-sphere motion # noqa: E501
    system using EPICS PVs (Process Variables). It is a subclass of PVPositionerIsClose
    and includes attributes for setting the setpoint, reading back the current position,
    actuating the motion, and stopping the motion.

    Attributes:
        setpoint (EpicsSignal): An EPICS signal for setting the desired position of
            the axis.
        readback (EpicsSignal): An EPICS signal for reading back the current position
            of the axis.
        actuate (EpicsSignal): An EPICS signal for initiating the motion of the axis.
        actuate_value (any, optional): The value to be set on the 'actuate' signal to initiate
            motion (default is 1).
        stop_signal (EpicsSignal): An EPICS signal for stopping the motion of the axis.
        stop_value (any, optional): The value to be set on the 'stop_signal' to stop motion
            (default is 1).

    Args:
        prefix (str): The EPICS PV prefix for this axis.
        axis (str): The axis identifier (e.g., 'X', 'Y', 'Z', 'rX', 'rY', 'rZ').
    ...
    Methods:
        move(position, wait=True, timeout=None, moved_cb=None, sync_enable=True):
...
class SQR1(Device):
    """
    This class represents the entire square-one tri-sphere motion system, which consists of # noqa: E501
    multiple axes (X, Y, Z, rX, rY, rZ) that can be controlled individually. It aggregates
    instances of the `SQR1Axis` class for each axis and provides methods for multi-axis
    movement and stopping the motion.

    Attributes:
        x (SQR1Axis): An instance of the `SQR1Axis` class representing the X-axis.
        y (SQR1Axis): An instance of the `SQR1Axis` class representing the Y-axis.
        z (SQR1Axis): An instance of the `SQR1Axis` class representing the Z-axis.
        rx (SQR1Axis): An instance of the `SQR1Axis` class representing the rX-axis.
        ry (SQR1Axis): An instance of the `SQR1Axis` class representing the rY-axis.
        rz (SQR1Axis): An instance of the `SQR1Axis` class representing the rZ-axis.
        stop_signal (EpicsSignal): An EPICS signal for stopping the motion of the entire
            tri-sphere motion system.

    Args:
        prefix (str): The EPICS PV prefix for the motion system.
        name (str): The name for this device.
        kind (str or None, optional): The kind of device (e.g., 'detector', 'motor').
        read_attrs (list or None, optional): List of attribute names to be read when
            reading this device.
        configuration_attrs (list or None, optional): List of attribute names to be
            configured when configuring this device.
        parent (Device or None, optional): The parent device, if applicable.
        **kwargs: Additional keyword arguments to pass to the base class constructor.

    Methods:
        sync_setpoints(): Synchronize the setpoints of all axes to their respective
            readback values.
        multi_axis_move(x_sp=None, y_sp=None, z_sp=None, rx_sp=None, ry_sp=None, rz_sp=None,
                        wait=True, timeout=10.0): Move multiple axes simultaneously to
            specified setpoints.
    ...
    def multi_axis_move(self,
        ...
        status = x_status & y_status & z_status & rx_status & ry_status & rz_status  # noqa: E501
        if wait:
            status_wait(status)
