...
class PDUChannel(Device):
    ...
    ch_ctrl_state = FCpt(
        EpicsSignalRO, "{self.prefix}:Outlet:{self._ch}:{self._ch_ctrl_state}", kind="hinted", string=True
    )
    ch_ctrl_command_on = FCpt(
        EpicsSignal, "{self.prefix}:Outlet:{self._ch}:{self._ch_command_on}", kind="omitted", string=True
    )
    ch_ctrl_command_off = FCpt(
        EpicsSignal, "{self.prefix}:Outlet:{self._ch}:{self._ch_command_off}", kind="omitted", string=True
    )
    ch_ctrl_command_reboot = FCpt(
        EpicsSignal, "{self.prefix}:Outlet:{self._ch}:{self._ch_command_reboot}", kind="omitted", string=True
    )
...
class TripplitePDUChannel(Device):
    ...
    ch_ctrl_command = FCpt(
        EpicsSignal, "{self.prefix}:Outlet:{self._ch}:{self._ch_command}", kind="hinted", string=True
    )
