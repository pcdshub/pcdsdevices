...
class WaveFrontSensorTarget(BaseInterface, GroupDevice, LightpathInOutCptMixin):
    """
    ...
    target = Cpt(
        WaveFrontSensorStates, ":MMS:STATE", kind="hinted", doc="Control of the diagnostic stack via saved positions."
    )
    y_motor = Cpt(BeckhoffAxisNoOffset, ":MMS:Y", kind="normal", doc="Direct control of the diagnostic stack motor.")
    z_motor = Cpt(BeckhoffAxisNoOffset, ":MMS:Z", kind="normal", doc="Z position of target stack for focus control.")
