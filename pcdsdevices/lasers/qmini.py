...
class QminiSpectrometer(Device):
    ...
    exposure = Cpt(EpicsSignal, ":GET_EXPOSURE_TIME", write_pv=":SET_EXPOSURE_TIME", kind="config")
    trig_mode = Cpt(EpicsSignal, ":TRIG_MODE_RBV", write_pv=":TRIG_MODE", kind="config")
    trig_delay = Cpt(EpicsSignal, ":GET_TRIG_DELAY", write_pv=":SET_TRIG_DELAY", kind="config")
    trig_pin = Cpt(EpicsSignal, ":TRIG_PIN_RBV", write_pv=":SET_TRIG_PIN", kind="config")
    trig_edge = Cpt(EpicsSignal, ":TRIG_EDGE_RBV", write_pv=":SET_TRIG_EDGE", kind="config")
    trig_enable = Cpt(EpicsSignal, ":GET_TRIG_ENABLE", write_pv=":SET_TRIG_ENABLE", kind="config")
    ...
    correct_nonlinearity = Cpt(EpicsSignal, ":CORRECT_NONLINEARITY", kind="omitted")
    remove_bad_pixels = Cpt(EpicsSignal, ":REMOVE_BAD_PIXELS", kind="omitted")
    subtract_dark = Cpt(EpicsSignal, ":SUBTRACT_DARK", kind="omitted")
    remove_temp_bad_pixels = Cpt(EpicsSignal, ":REMOVE_TEMP_BAD_PIXELS", kind="omitted")
    normalize_exposure = Cpt(EpicsSignal, ":NORMALIZE_EXPOSURE", kind="omitted")
    sensitivity_cal = Cpt(EpicsSignal, ":SENSITIVITY_CAL", kind="omitted")
    correct_prnu = Cpt(EpicsSignal, ":CORRECT_PRNU", kind="omitted")
    additional_filtering = Cpt(EpicsSignal, ":ADDITIONAL_FILTERING", kind="omitted")
    scale_to_16_bit = Cpt(EpicsSignal, ":SCALE_TO_16BIT", kind="omitted")
    set_processing_steps = Cpt(EpicsSignal, ":SET_PROCESSING_STEPS", kind="omitted")
    get_processing_steps = Cpt(EpicsSignalRO, ":GET_PROCESSING_STEPS", kind="omitted")
    ...
    def save_data(self, file_dest: str = ""):
        ...
        _data = {
            ...
            "settings": {f"{sig}": bool(getattr(self, sig).get()) for sig in _settings},
...
class QminiWithEvr(QminiSpectrometer):
    ...
    event_code = FCpt(
        EpicsSignal,
        "{self._evr_pv}:TRIG{self._evr_ch}:EC_RBV",
    ...
    evr_delay = FCpt(
        EpicsSignal,
        "{self._evr_pv}:TRIG{self._evr_ch}:BW_TDES",
