...
class ItechRfofRx(BaseInterface, Device):
    ...
    internal_lock = Cpt(
        EpicsSignalRO, ":RX_PC1", kind="normal", doc="Rx internal lock (uV)"
    )
    ...
    output_rf_power = Cpt(
        EpicsSignalRO, ":RX_RFOUT", kind="normal", doc="Output RF power (dBm)"
    )
    ...
    spool_temp = Cpt(
        EpicsSignalRO,
        ":RX_SPL_T", kind="normal", doc="Rx spool temperature (C)"
    )
    ext_relative_hum = Cpt(
        EpicsSignalRO,
        ":RX_EXT_H", kind="normal", doc="External relative humidity (%)"
    )
    ...
    ext_press = Cpt(
        EpicsSignalRO,
        ":RX_EXT_P", kind="normal", doc="External pressure (mbar)"
    )
    ...
    int_relative_hum = Cpt(
        EpicsSignalRO,
        ":RX_INT_H", kind="normal", doc="Internal relative humidity (%)"
    )
    ...
    int_press = Cpt(
        EpicsSignalRO,
        ":RX_INT_P", kind="normal", doc="Internal pressure (mbar)"
    )
...
class ItechRfofTx(BaseInterface, Device):
    ...
    las_diode_sp = Cpt(
        EpicsSignalRO,
        ":TX_LD_S", kind="normal", doc="Laser diode setpoint (uA)"
    )
    las_temp = Cpt(
        EpicsSignalRO,
        ":TX_LAS_T", kind="normal", doc="Laser temperature (C)"
    )
    comp_spool_temp = Cpt(
        EpicsSignalRO,
        ":TX_SPL_T",
    ...
    input_rf_power = Cpt(
        EpicsSignalRO,
        ":TX_RFIN", kind="normal", doc="Input RF power (dBm)"
    )
    ...
    las_opt_power = Cpt(
        EpicsSignalRO,
        ":TX_LAS_P", kind="normal", doc="Laser optical power (mW)"
    )
    rf_output_mzm_power = Cpt(
        EpicsSignalRO,
        ":TX_RFO", kind="normal", doc="RF output power to MZM (dBm)"
    )
    las_current = Cpt(
        EpicsSignalRO,
        ":TX_LAS_I", kind="normal", doc="Laser current (mA)"
    )
    ...
    input_phase_shift_v = Cpt(
        EpicsSignalRO,
        ":TX_PSI_V",
        kind="normal", doc="Input phase shifter voltage (V)"
    )
    mzm_bias_v = Cpt(
        EpicsSignalRO,
        ":TX_MZM_V",
        kind="normal", doc="MZ modulator bias voltage (V)"
    )
    voa_control_sp = Cpt(
        EpicsSignalRO,
        ":TX_VOA_S",
...
class ItechRfofStatus(BaseInterface, Device):
    ...
    ready = Cpt(
        EpicsSignalRO,
        ":READY", kind="normal", doc="Binary showing if device is ready"
    )
...
class ItechRfofErrors(BaseInterface, Device):
    ...
    rf_pwr_err = Cpt(
        EpicsSignalRO,
        ":RF_PWR_ERR", kind="normal", doc="RF power is out of range"
    )
    opt_pwr_err = Cpt(
        EpicsSignalRO,
        ":OPT_PWR_ERR", kind="normal", doc="Optical power is too low"
    )
    las_pwr_err = Cpt(
        EpicsSignalRO,
        ":LAS_PWR_ERR", kind="normal", doc="Laser is switched off"
    )
    phase_err = Cpt(
        EpicsSignalRO,
        ":PHASE_ERR", kind="normal", doc="Phase loops are not locked"
    )
    fan_spd_err = Cpt(
        EpicsSignalRO,
        ":FAN_SPD_ERR", kind="normal", doc="Fan revolutions are too low"
    )
