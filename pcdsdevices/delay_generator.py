...
TRIGGER_INHIBITS = {"0": "Off", "1": "Triggers", "2": "AB", "3": "AB,CD", "4": "AB,CD,EF", "5": "AB,CD,EF,GH"}
...
class DgChannel(BaseInterface, Device):
    ...
    reference = Cpt(EpicsSignal, "ReferenceMO", kind="normal", doc="Reference channel from which the delay taken.")
...
class DelayGeneratorBase(BaseInterface, Device):
    ...
    trig_source = Cpt(EpicsSignal, ":triggerSourceMI", write_pv=":triggerSourceMO", kind="config")
    trig_inhibit = Cpt(EpicsSignal, ":triggerInhibitMI", write_pv=":triggerInhibitMO", kind="config")

    tab_component_names = True
    tab_whitelist = [
        "print_trigger_sources",
        "get_trigger_source",
        "set_trigger_source",
        "print_trigger_inhibit",
        "get_trigger_inhibit",
        "set_trigger_inhibit",
    ]
...
for channel in CHANNELS[:-1]:
    channel_cpts[f"ch{channel}"] = Cpt(DgChannel, f":{channel.lower()}", name=f"ch{channel}")
