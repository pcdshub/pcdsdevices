...
class SliceDhvChannel(BaseInterface, Device):
    ...
    voltage_limit = Cpt(EpicsSignal, ":VLIM_GET", write_pv=":VLIM_SET", kind="config")

    control_settings = Cpt(EpicsSignal, ":CTL_SETTINGS_GET", write_pv=":CTL_SETTINGS_SET", kind="config")

    bias_voltage = Cpt(EpicsSignal, ":VMEAS_GET", write_pv=":VBIAS_SET", kind="normal")

    sweep_voltage = Cpt(EpicsSignal, ":VSWEEP_GET", write_pv=":VSWEEP_SET", kind="config")

    sweep_freq = Cpt(EpicsSignal, ":FSWEEP_GET", write_pv=":FSWEEP_SET", kind="config")

    sweep_mode = Cpt(EpicsSignal, ":SWEEP_MODE_GET", write_pv=":SWEEP_MODE_SET", kind="config")

    error_status = Cpt(EpicsSignalRO, ":ERROR", kind="config")

    trigger_in = Cpt(EpicsSignal, ":TRIGIN_STATUS_GET", write_pv=":TRIGIN_STATUS_SET", kind="config")

    trigger_out = Cpt(EpicsSignal, ":TRIGOUT_STATUS_GET", write_pv=":TRIGOUT_STATUS_SET", kind="config")

    modulation = Cpt(EpicsSignal, ":MODULATION_GET", write_pv=":MODULATION_SET", kind="config")

    output_mode = Cpt(EpicsSignal, ":OUTPUT_MODE_GET", write_pv=":OUTPUT_MODE_SET", kind="config")
